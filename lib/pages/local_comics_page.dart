...
class _LocalComicsPageState extends State<LocalComicsPage> {
  ...
  void exportComics(
    ...
    var loadingController = showLoadingDialog(
      context,
      allowCancel: true,
      message: "Exporting @current/@total".tlParams({'current': current, 'total': comics.length}),
    ...
    try {
      ...
      for (var comic in comics) {
        ...
        if (comics.length > 1) {
          loadingController
              .setMessage("Exporting @current/@total".tlParams({'current': current, 'total': comics.length}));
