...
class JsEngine with _JSEngineApi, JsUiApi, Init {
  ...
  Future<Map<String, dynamic>> _http(Map<String, dynamic> req) async {
    ...
    try {
      // 检查是否为可能导致问题的URL
      String url = req["url"] ?? "";
      if (_isProblematicUrl(url)) {
        Log.warning(
          "JS Engine",
          "Blocking potentially problematic request to: $url",
        );
        return {
          "status": 200,
          "headers": {"content-type": "text/html"},
          "body":
              "<html><body>Request blocked for performance reasons</body></html>",
          "error": null,
        };
      }
...
mixin class _JSEngineApi {
  ...
  Object? handleHtmlCallback(Map<String, dynamic> data) {
    switch (data["function"]) {
      case "parse":
        ...
        // 检查HTML文档大小，防止解析过大的文档导致卡顿
        String htmlContent = data["data"];
        const maxHtmlSize = 5 * 1024 * 1024; // 5MB限制

        if (htmlContent.length > maxHtmlSize) {
          Log.warning(
            "JS Engine",
            "HTML document too large (${htmlContent.length} bytes), truncating to ${maxHtmlSize} bytes to prevent blocking",
          );
          // 截取前面部分，但尽量保持HTML结构完整
          htmlContent = _truncateHtmlSafely(htmlContent, maxHtmlSize);
        }

        try {
          _documents[data["key"]] = DocumentWrapper.parse(htmlContent);
        } catch (e) {
          Log.error("JS Engine", "Failed to parse HTML document: $e");
          // 创建一个空文档作为fallback
          _documents[data["key"]] = DocumentWrapper.parse(
            "<html><body></body></html>",
          );
        }
        return null;
  ...
  bool _isProblematicUrl(String url) {
    // 默认启用Apple App Store URL过滤，因为这些请求经常导致卡顿
    // 用户可以通过设置 enableUrlFiltering = false 来禁用
    bool enableFiltering = appdata.settings['enableUrlFiltering'] ?? true;
    if (!enableFiltering) {
      return false;
    }

    // Apple App Store URLs - 这些通常返回非常大的HTML文档
    if (url.contains('apps.apple.com')) {
      return true;
    }

    // Google Play Store URLs - 类似的问题
    if (url.contains('play.google.com')) {
      return true;
    }

    // 其他已知的大型网站
    List<String> problematicDomains = [
      'facebook.com',
      'twitter.com',
      'instagram.com',
      'youtube.com',
    ...
    for (String domain in problematicDomains) {
  ...
  /// 安全地截取HTML内容，尽量保持结构完整
  String _truncateHtmlSafely(String html, int maxLength) {
    if (html.length <= maxLength) return html;

    // 找到一个合适的截取点，避免在标签中间截断
    int cutPoint = maxLength;

    // 向前查找最近的标签结束位置
    for (int i = maxLength; i >= maxLength - 1000 && i >= 0; i--) {
      if (html[i] == '>') {
        cutPoint = i + 1;
        break;
      }
    ...
    String truncated = html.substring(0, cutPoint);

    // 简单的标签平衡检查，确保基本的HTML结构
    if (!truncated.contains('</body>')) {
      truncated += '</body>';
    }
    if (!truncated.contains('</html>')) {
      truncated += '</html>';
    }
...
class DocumentWrapper {
  ...
  DocumentWrapper.parse(String htmlContent)
    : doc = _parseHtmlSafely(htmlContent);

  /// 安全地解析HTML，添加错误处理和性能优化
  static dom.Document _parseHtmlSafely(String htmlContent) {
    try {
      // 对于特别大的文档，使用更宽松的解析选项
      if (htmlContent.length > 1024 * 1024) {
        // 1MB
        // 可以在这里添加更多的预处理逻辑
        // 比如移除不必要的script标签、style标签等来减少解析负担
        htmlContent = _preprocessLargeHtml(htmlContent);
      }

      return html.parse(htmlContent);
    } catch (e) {
      Log.error("DocumentWrapper", "Failed to parse HTML: $e");
      // 返回一个最小的有效HTML文档
      return html.parse("<html><head></head><body></body></html>");
    }
  }

  /// 预处理大型HTML文档，移除可能导致性能问题的内容
  static String _preprocessLargeHtml(String htmlContent) {
    // 移除大型的script标签内容，保留标签结构
    htmlContent = htmlContent.replaceAllMapped(
      RegExp(r'<script[^>]*>[\s\S]*?</script>', caseSensitive: false),
      (match) {
        String tag = match.group(0)!;
        // 保留script标签但清空内容，避免解析大量JavaScript
        if (tag.length > 10000) {
          return tag.replaceAllMapped(
            RegExp(r'(<script[^>]*>)[\s\S]*?(</script>)', caseSensitive: false),
            (m) =>
                '${m.group(1)}/* content removed for performance */${m.group(2)}',
          );
        }
        return tag;
      },
      ...
      (match) {
        ...
        if (tag.length > 50000) {
          return tag.replaceAllMapped(
            RegExp(r'(<style[^>]*>)[\s\S]*?(</style>)', caseSensitive: false),
            (m) =>
                '${m.group(1)}/* styles removed for performance */${m.group(2)}',
          );
        }
        return tag;
      },
