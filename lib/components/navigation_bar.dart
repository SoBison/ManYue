          ...
          children: [
            ...
            SizedBox(
              ...
              child: Row(
                ...
                children: List<Widget>.generate(widget.paneItems.length, (
                  index,
                ) {
                  return _SingleBottomNaviWidget(
...
class _SingleBottomNaviWidgetState extends State<_SingleBottomNaviWidget>
  ...
  Widget buildContent() {
    ...
    // 使用更现代的图标
    IconData getModernIcon() {
      if (widget.entry.label == '首页') {
        return widget.enabled ? Icons.home_rounded : Icons.home_outlined;
      } else if (widget.entry.label == '收藏' || widget.entry.label == '最爱') {
        return widget.enabled
            ? Icons.favorite_rounded
            : Icons.favorite_outline_rounded;
      } else if (widget.entry.label == '历史') {
        return widget.enabled ? Icons.history_rounded : Icons.history;
      } else if (widget.entry.label == '设置') {
        return widget.enabled
            ? Icons.settings_rounded
            : Icons.settings_outlined;
      } else if (widget.entry.label == '搜索') {
        return widget.enabled ? Icons.search_rounded : Icons.search_outlined;
      } else {
        return widget.enabled ? widget.entry.activeIcon : widget.entry.icon;
      }
    }
        ...
        child: Stack(
          ...
          children: [
            ...
            TweenAnimationBuilder<double>(
              tween: Tween<double>(
                begin: widget.enabled ? 0.0 : 1.0,
                end: widget.enabled ? 1.0 : 0.0,
              ),
